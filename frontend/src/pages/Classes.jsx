...
const LEVEL_BG = ['from-purple-600/20 to-violet-800/10', 'from-amber-600/20 to-yellow-700/10', 'from-cyan-600/20 to-sky-700/10'];
...
export default function Classes() {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        setVisible(settingsRes.data.classes_visible);
  ...
  const selectLevel = async (level) => {
    ...
    setLoadingSub(true);
    try {
      const r = await api.get(`/classes/levels/${level.id}/subjects`);
  ...
  const selectSubject = async (subject) => {
    ...
    setLoadingCh(true);
    try {
      const r = await api.get(`/classes/subjects/${subject.id}/chapters`);
  ...
  const goBack = () => {
    ...
    if (currentSubject) { setCurrentSubject(null); setCurrentChapter(null); return; }
    if (currentLevel) { setCurrentLevel(null); setCurrentSubject(null); setCurrentChapter(null); }
  };
  ...
  if (!visible) return (
    <div className="flex flex-col items-center justify-center py-24 text-center px-4">
      ...
      <p className="text-white/30 font-medium">Classes are currently unavailable</p>
      <p className="text-white/15 text-sm mt-1">Check back later</p>
    </div>
  );

  const embedUrl = currentChapter ? getYouTubeEmbedUrl(currentChapter.youtube_url) : null;

  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      {/* Breadcrumb */}
      <div className="flex items-center gap-1.5 text-sm mb-6 flex-wrap">
        <button onClick={() => { setCurrentLevel(null); setCurrentSubject(null); setCurrentChapter(null); }}
          className={`font-medium transition-colors ${!currentLevel ? 'text-white' : 'text-white/40 hover:text-white/70'}`}>
          <span className="flex items-center gap-1.5"><Layers className="w-4 h-4" /> Classes</span>
        </button>
        {currentLevel && (
          <>
      ...
      {/* VIEW: CHAPTER (Video Player) */}
      {currentChapter && (
        <motion.div key="chapter-view" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          <div className="card-premium overflow-hidden">
            {embedUrl ? (
              ...
              <div className="flex flex-col items-center justify-center bg-white/[0.02] py-20">
                <div className="w-16 h-16 rounded-2xl bg-red-500/10 flex items-center justify-center mb-4">
                  <Youtube className="w-8 h-8 text-red-400/50" />
                </div>
                <p className="text-white/30 font-medium">No video available</p>
                <p className="text-white/15 text-sm mt-1">This chapter doesn't have a video yet</p>
              </div>
            )}
            <div className="p-5">
              <h1 className="text-white font-bold text-xl mb-2">{currentChapter.title}</h1>
              ...
              {chapters.map((ch, i) => {
                ...
                return (
                  <motion.button key={ch.id} onClick={() => selectChapter(ch)}
                    ...
                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform group-hover:scale-105 ${hasVideo ? 'bg-red-500/10' : 'bg-white/[0.04]'}`}>
                      {hasVideo ? <Play className="w-5 h-5 text-red-400" /> : <Lock className="w-4 h-4 text-white/20" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      {!hasVideo && <p className="text-white/20 text-[10px] mt-0.5">No video available</p>}
                    </div>
      ...
      {!currentLevel && (
        <motion.div key="levels-view" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          <div className="mb-6">
            <h1 className="text-white font-bold text-2xl">Classes</h1>
            <p className="text-white/35 text-sm mt-1">Choose your level to get started</p>
          </div>
          {levels.length === 0 ? (
            <div className="text-center py-20 card-premium">
              ...
              <p className="text-white/30 font-medium">No classes available yet</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {levels.map((level, i) => (
                <motion.button key={level.id} onClick={() => selectLevel(level)}
                  ...
                  className={`card-premium p-6 text-left group hover:-translate-y-1 transition-all bg-gradient-to-br ${getLevelBg(i)} hover:border-white/20`}>
